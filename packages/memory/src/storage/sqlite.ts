...
export class KnowledgeDB {
  ...
  async init(): Promise<void> {
    ...
    try {
      await this.db.execute(
        "CREATE VIRTUAL TABLE IF NOT EXISTS memory_fts USING fts5(fact, content=memory, content_rowid=id)",
      );
    } catch {
      await this.db.execute(
        "CREATE VIRTUAL TABLE IF NOT EXISTS memory_fts USING fts5(fact, content_rowid=id)",
      );
    }
  ...
  async remember(fact: string, type: string): Promise<MemoryEntry> {
    await this.init();
  ...
  async update(id: number, fact: string, type: string): Promise<MemoryEntry | null> {
    await this.init();
  ...
  async stats(): Promise<{
    ...
    const typeRows = await this.db.execute(
      "SELECT type, COUNT(*) as c FROM memory GROUP BY type",
    );
