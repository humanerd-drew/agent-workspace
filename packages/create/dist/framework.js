...
export function getMcpServerConfig(cwd, install, global) {
    const dbPath = global
...
export function injectMcpConfig(framework, cwd, config) {
    switch (framework) {
        case "opencode":
...
function injectMcpEntry(configPath, serverName, entry) {
...
function injectOpencodeConfig(cwd, config) {
    return injectMcpEntry(join(cwd, "opencode.jsonc"), "agent-memory", toOpencodeFormat(config));
}
function injectClaudeCodeConfig(cwd, config) {
    ...
    if (existsSync(configPath)) {
        ...
        if (raw) {
            ...
            if (!parsed.mcpServers)
                parsed.mcpServers = {};
            if (parsed.mcpServers["agent-memory"])
                return true;
...
function injectCursorConfig(cwd, config) {
...
export function getAgentsMdPath(cwd) {
