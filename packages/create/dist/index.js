...
import { detectFramework, getMcpServerConfig, injectMcpConfig, getAgentsMdPath, } from "./framework.js";
...
function readAndRender(filePath, vars) {
    const content = readFileSync(filePath, "utf-8");
    return renderTemplate(content, vars);
}
function createAgentDir(cwd) {
...
function writeAgentFiles(agentDir, vars) {
...
function generateAgentsMd(vars) {
...
function writeOrUpdateAgentsMd(cwd, vars, framework) {
...
function printSummary(agentDir, framework, configInjected, installed, isGlobal) {
    console.log(`
  ┌─────────────────────────────────────┐
...
async function main() {
    ...
    const detection = opts.framework === "auto"
        ? detectFramework(cwd)
        : { framework: opts.framework, confidence: 1 };
